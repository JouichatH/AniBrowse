...
_HANDSHAKE_EDITS: dict[str, list[tuple[str, str]]] = {
    ...
    'provider.py': [
        ('import logging\nfrom json import dumps\nfrom typing import Any, Iterator\n\n',
         'import logging\nimport threading\nimport time\nfrom json import dumps\nfrom typing import Any, Iterator\n\n'),
        ('logger = logging.getLogger(__name__)\n\n\n',
         'logger = logging.getLogger(__name__)\n\n# The episode endpoint rate-limits bursts ("Too many requests, please try\n# again in 5 seconds") - and the app naturally bursts: the foreground fetch\n# plus TWO neighbour prefetches fire within a couple of seconds, so the\n# prefetches got rejected and cached the nyaa fallback instead (seen live\n# 2026-07-24). Pace episode requests app-wide, and honour the server\'s ask\n# when a burst still slips through.\n_MIN_EPISODE_REQUEST_SPACING = 2.5  # seconds between episode requests\n_RATE_LIMIT_WAIT = 6.0  # server asks for 5s; give it a margin\n_pace_lock = threading.Lock()\n_last_episode_request = 0.0\n_sleep = time.sleep  # indirection so tests can stub the waits\n\n\ndef _pace_episode_request() -> None:\n    """Block (briefly) so consecutive episode requests keep their spacing.\n\n    Called from the foreground fetch and the background prefetch threads\n    alike; the foreground almost always goes first (prefetch starts as the\n    player launches), so any visible wait lands on background threads.\n    """\n    global _last_episode_request\n    with _pace_lock:\n        wait = _MIN_EPISODE_REQUEST_SPACING - (time.monotonic() - _last_episode_request)\n        if wait > 0:\n            _sleep(wait)\n        _last_episode_request = time.monotonic()\n\n\n'),
