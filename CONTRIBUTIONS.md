# Contributing to Ani-Browse

First off, thank you for considering contributing to Ani-Browse! We welcome any help, whether it's reporting a bug, proposing a feature, or writing code. This document will guide you through the process.

## How Can I Contribute?

There are many ways to contribute to the Ani-Browse project:

*   **Reporting Bugs:** If you find a bug, please create an issue in our [issue tracker](https://github.com/viu-media/Ani-Browse/issues).
...
*   **Adding a Provider, Player, or Selector:** Extend Ani-Browse's capabilities by integrating new tools and services.

## Contribution Workflow

We follow the standard GitHub Fork & Pull Request workflow.

1.  **Create an Issue:** Before starting work on a new feature or a significant bug fix, please [create an issue](https://github.com/viu-media/Ani-Browse/issues/new/choose) to discuss your idea. This allows us to give feedback and prevent duplicate work. For small bugs or documentation typos, you can skip this step.

2.  **Fork the Repository:** Create your own fork of the Ani-Browse repository.

3.  **Clone Your Fork:**
    ```bash
    git clone https://github.com/YOUR_USERNAME/Ani-Browse.git
    cd Ani-Browse
    ```
...
9.  **Submit a Pull Request:** Open a pull request from your branch to the `master` branch of the main Ani-Browse repository. Provide a clear title and description of your changes.
...
*   **Modularity and Architecture:**
    ...
    *   **Configuration:** All configuration is managed through Pydantic models in `ani-browse/core/config/model.py`. When adding new config options, update the model, defaults, and descriptions.
...
1.  **Create a New Provider Directory:** Inside `ani-browse/libs/provider/anime/`, create a new directory with the provider's name (e.g., `ani-browse/libs/provider/anime/newprovider/`).
...
2.  **Implement the Provider:**
    ...
    *   Create `mappers.py` to convert the provider's data structures into the generic types defined in `ani-browse/libs/provider/anime/types.py`.
...
3.  **Register the Provider:**
    *   Add your new provider to the `ProviderName` enum in `ani-browse/libs/provider/anime/types.py`.
    *   Register it in the `PROVIDERS_AVAILABLE` dictionary in `ani-browse/libs/provider/anime/provider.py`.

4.  **Add Normalization Rules (Optional):** If the provider uses different anime titles than AniList, add mappings to `ani-browse/assets/normalizer.json`.

## How to Add a New Player

1.  **Create a New Player Directory:** Inside `ani-browse/libs/player/`, create a directory for your player (e.g., `ani-browse/libs/player/myplayer/`).
...
3.  **Add Configuration (if needed):**
    *   If your player has configurable options, add a new Pydantic model (e.g., `MyPlayerConfig`) in `ani-browse/core/config/model.py`. It should inherit from `OtherConfig`.
...
4.  **Register the Player:**
    *   Add your player's name to the `PLAYERS` list in `ani-browse/libs/player/player.py`.
    *   Add the logic to instantiate your player class within the `PlayerFactory.create` method.

## How to Add a New Selector

1.  **Create a New Selector Directory:** Inside `ani-browse/libs/selectors/`, create a new directory (e.g., `ani-browse/libs/selectors/myselector/`).
...
4.  **Register the Selector:**
    *   Add your selector's name to the `SELECTORS` list in `ani-browse/libs/selectors/selector.py`.
    *   Add the instantiation logic to the `SelectorFactory.create` method.
    *   Update the `Literal` type hint for the `selector` field in `GeneralConfig` (`ani-browse/core/config/model.py`).

## How to Add a New CLI Command or Service

Our CLI uses `click` and a `LazyGroup` class to load commands on demand.

### Adding a Top-Level Command (e.g., `ani-browse my-command`)

1.  **Create the Command File:** Create a new Python file in `ani-browse/cli/commands/` (e.g., `my_command.py`). This file should contain your `click.command()` function.

2.  **Register the Command:** In `ani-browse/cli/cli.py`, add your command to the `commands` dictionary.
    ```python
    commands = {
        # ... existing commands
...
### Adding a Subcommand (e.g., `ani-browse anilist my-subcommand`)

1.  **Create the Command File:** Place your new command file inside the appropriate subdirectory, for example, `ani-browse/cli/commands/anilist/commands/my_subcommand.py`.

2.  **Register the Subcommand:** In the parent command's entry point file (e.g., `ani-browse/cli/commands/anilist/cmd.py`), add your subcommand to the `commands` dictionary within the `LazyGroup`.
    ```python
    @click.group(
        cls=LazyGroup,
        # ... other options
...
### Creating a Service
If your command involves complex logic, consider creating a service in `ani-browse/cli/service/` to keep the business logic separate from the command-line interface. This service can then be instantiated and used within your `click` command function. This follows the existing pattern for services like `DownloadService` and `PlayerService`.

---
Thank you for contributing to Ani-Browse
